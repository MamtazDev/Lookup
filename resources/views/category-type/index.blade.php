    ...
    <div class="card-header">
               ...
               @can('category-type-create')
                 <span class="float-right">
                        <a class="btn btn-primary btn-radius" href="{{ route('category-type.create') }}">Add new Category type</a>
                    </span>
               @endcan
              </div>
               <div class="card-body">
                    ...
                    @enderror
                                ...
                                <td>
                                    ...
                                    @can('category-type-edit')
                                        <a class="btn btn-primary" href="{{ route('category-type.edit',$type->id) }}"><i class="fa fa-edit"></i> </a>
                                    @endcan
                                    @can('category-type-delete')
                                    <form method="POST" action="{{ route('category-type.destroy', $type->id) }}" style="display: inline-block">
                            @csrf
...
<script>
   ...
   var status = $(this).prop('checked') == true ? 1 : 0; 
        ...
        $.ajax({
            type: "POST",
            dataType: "json",
            url: "{{ route('categorytypechangestatus') }}",
