    ...
    <div class="card-header">
               ...
               @can('colors-create')
                 <span class="float-right">
                        <a class="btn btn-primary btn-radius" href="{{ route('colors.create') }}">Add new Color</a>
                    </span>
               @endcan
              </div>
               <div class="card-body">
                <table class="table table-hover" id="datatable">
                   ...
                   @error('status')
                    ...
                    @enderror
                                ...
                                <td>
                                    @can('colors-edit')
                                        <a class="btn btn-primary" href="{{ route('colors.edit',$type->id) }}"><i class="fa fa-edit"></i> </a>
                                    @endcan
                                    @can('colors-delete')
                                    <form method="POST" action="{{ route('colors.destroy', $type->id) }}" style="display: inline-block">
                            @csrf
...
<script>
   ...
   var status = $(this).prop('checked') == true ? 1 : 0; 
        ...
        $.ajax({
            type: "POST",
            dataType: "json",
            url: "{{ route('colorcodeschangestatus') }}",
