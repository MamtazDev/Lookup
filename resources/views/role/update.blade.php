        ...
        <div class="card">
            <div class="card-header">
                <span class="float-right">
                    <a class="btn btn-primary btn-radius" href="{{ route('role.index') }}">Roles</a>
                </span>
            </div>
            <div class="card-body">
                {!! Form::model($role, ['route' => ['role.update', $role->id],'method' => 'PATCH']) !!}
                  ...
                  <!--   <table class="table table-hover">
                      ...
                      <tbody>
                        ...
                        <tr>
                          <td>{{$key+1}}</td>
                          <td>@if((substr($value->name,-6)=='create') && !(substr($value->name,-6)=='delete'))
                            {{ Form::checkbox('permission[]', $value->id, in_array($value->id, $rolePermissions) ? true : false, array('class' => 'name')) }}
                            {{ $value->name }}
                            @endif</td>
                          <td></td>
                          ...
                          <td></td>
                          <td>
                            @if(substr($value->name,-4)=='list')
                            {{ Form::checkbox('permission[]', $value->id, in_array($value->id, $rolePermissions) ? true : false, array('class' => 'name')) }}
                            {{ $value->name }}
                            @endif
                         
                          @if(substr($value->name,-4)=='edit')
                            {{ Form::checkbox('permission[]', $value->id, in_array($value->id, $rolePermissions) ? true : false, array('class' => 'name')) }}
                            {{ $value->name }}
                            @endif
                          @if(substr($value->name,-6)=='create')
                            {{ Form::checkbox('permission[]', $value->id, in_array($value->id, $rolePermissions) ? true : false, array('class' => 'name')) }}
                            {{ $value->name }}
                            @endif
                          @if(substr($value->name,-6)=='delete')
                            {{ Form::checkbox('permission[]', $value->id, in_array($value->id, $rolePermissions) ? true : false, array('class' => 'name')) }}
                            {{ $value->name }}
                            @endif</td>
                          </tr>
